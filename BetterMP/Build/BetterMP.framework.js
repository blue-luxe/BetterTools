...
function _DownloadFile(array, size, fileNamePtr) {
...
function _GetVoxFile(LoadMod, JsonjsModel, type) {
 function VOX(X, Y, Z) {
  ...
  this.palette = [];
  for (var i = 256; --i > -1; ) {
   this.palette.push(4278190080 | i | i << 8 | i << 16);
  }
  ...
  this.appendUInt32 = function(data, n) {
   data.push(n & 255, n >>> 8 & 255, n >>> 16 & 255, n >>> 24 & 255);
  };
  this.appendRGBA = function(data, n) {
   data.push(n >>> 16 & 255, n >>> 8 & 255, n & 255, n >>> 24 & 255);
  };
  ...
  this.export = function(filename) {
   ...
   this.appendUInt32(data, this.vcount * 4 + 1076);
 ...
 const zip = new JSZip();
 const json = UTF8ToString(JsonjsModel);
 const obj = JSON.parse(json);
 var val = 0;
 var typestring = UTF8ToString(type);
 if (typestring == "HUMAN") {
  var voxSizex = 40;
  var voxSizey = 40;
  var voxSizez = 40;
  var vox = new VOX(voxSizex, voxSizey, voxSizez);
  CreateVoxDetal(vox, obj.LEGS, obj.LEGSId, 13, 14, 0);
  if (LoadMod == 0) {
   vox.export(obj.LEGSId + ".vox");
   vox = new VOX(voxSizex, voxSizey, voxSizez);
  }
  CreateVoxDetalClone(vox, obj.LEGS, obj.LEGSId, 22, 14, 0, 4);
  if (LoadMod == 0) {
   vox.export(obj.LEGSId + "(1).vox");
   vox = new VOX(voxSizex, voxSizey, voxSizez);
  }
  CreateVoxDetal(vox, obj.BODY, obj.BODYId, 11, 11, 9);
  if (LoadMod == 0) {
   vox.export(obj.BODYId + ".vox");
   vox = new VOX(voxSizex, voxSizey, voxSizez);
  }
  CreateVoxDetal(vox, obj.ARMS, obj.ARMSId, 9, 15, 12);
  if (LoadMod == 0) {
   vox.export(obj.ARMSId + ".vox");
   vox = new VOX(voxSizex, voxSizey, voxSizez);
  }
  CreateVoxDetalClone(vox, obj.ARMS, obj.ARMSId, 26, 15, 12, 4);
  if (LoadMod == 0) {
   vox.export(obj.ARMSId + "(1).vox");
   vox = new VOX(voxSizex, voxSizey, voxSizez);
  }
  CreateVoxDetal(vox, obj.HEAD, obj.HEADId, 11, 11, 20);
  if (LoadMod == 0) {
   vox.export(obj.HEADId + ".vox");
   zip.generateAsync({
    type: "blob"
   }).then(function(content) {
    saveAs(content, obj.HEADId + obj.ARMSId + obj.BODYId + obj.LEGSId + ".zip");
   });
  } else {
   vox.export(obj.HEADId + obj.ARMSId + obj.BODYId + obj.LEGSId + ".vox");
  }
 } else if (typestring == "ANIMAL") {
  var voxSizex = 60;
  var voxSizey = 60;
  var voxSizez = 60;
  var vox = new VOX(voxSizex, voxSizey, voxSizez);
  CreateVoxDetal(vox, obj.P_FLEG, obj.P_FLEGId, 15, 11, 0);
  if (LoadMod == 0) {
   vox.export(obj.P_FLEGId + ".vox");
   vox = new VOX(voxSizex, voxSizey, voxSizez);
  }
  CreateVoxDetalClone(vox, obj.P_FLEG, obj.P_FLEGId, 21, 11, 0, 3);
  if (LoadMod == 0) {
   vox.export(obj.P_FLEGId + "(1).vox");
   vox = new VOX(voxSizex, voxSizey, voxSizez);
  }
  CreateVoxDetal(vox, obj.P_BLEG, obj.P_BLEGId, 14, 18, 0);
  if (LoadMod == 0) {
   vox.export(obj.P_BLEGId + ".vox");
   vox = new VOX(voxSizex, voxSizey, voxSizez);
  }
  CreateVoxDetalClone(vox, obj.P_BLEG, obj.P_BLEGId, 22, 18, 0, 3);
  if (LoadMod == 0) {
   vox.export(obj.P_BLEGId + "(1).vox");
   vox = new VOX(voxSizex, voxSizey, voxSizez);
  }
  CreateVoxDetal(vox, obj.P_BODY, obj.P_BODYId, 14, 10, 5);
  if (LoadMod == 0) {
   vox.export(obj.P_BODYId + ".vox");
   vox = new VOX(voxSizex, voxSizey, voxSizez);
  }
  CreateVoxDetal(vox, obj.P_HEAD, obj.P_HEADId, 14, 8, 14);
  if (LoadMod == 0) {
   vox.export(obj.P_HEADId + ".vox");
   vox = new VOX(voxSizex, voxSizey, voxSizez);
  }
  CreateVoxDetal(vox, obj.P_TAIL, obj.P_TAILId, 15, 20, 14);
  if (LoadMod == 0) {
   vox.export(obj.P_TAILId + ".vox");
   zip.generateAsync({
    type: "blob"
   }).then(function(content) {
    saveAs(content, obj.P_BLEGId + obj.P_BODYId + obj.P_FLEGId + obj.P_HEADId + obj.P_TAILId + ".zip");
   });
  } else {
   vox.export(obj.P_BLEGId + obj.P_BODYId + obj.P_FLEGId + obj.P_HEADId + obj.P_TAILId + ".vox");
  }
 } else if (typestring == "16X") {
  var vox = new VOX(18, 18, 18);
  CreateVoxDetal(vox, obj.vox_model_data.vox_unit_datas, obj.id, 0, 0, 0);
  if (LoadMod == 0) {
   vox.export(obj.id + ".vox");
   zip.generateAsync({
    type: "blob"
   }).then(function(content) {
    saveAs(content, obj.id + ".zip");
   });
  } else {
   vox.export(obj.id + ".vox");
  }
  ...
  var voxSizez = 40;
  var vox = new VOX(voxSizex, voxSizey, voxSizez);
  CreateVoxDetal(vox, obj.LEGS, obj.LEGSId, 15, 14, 0);
  if (LoadMod == 0) {
   vox.export(obj.LEGSId + ".vox");
   vox = new VOX(voxSizex, voxSizey, voxSizez);
  }
  CreateVoxDetalClone(vox, obj.LEGS, obj.LEGSId, 20, 14, 0, 4);
  if (LoadMod == 0) {
   vox.export(obj.LEGSId + "(1).vox");
   vox = new VOX(voxSizex, voxSizey, voxSizez);
  }
  CreateVoxDetal(vox, obj.BODY, obj.BODYId, 15, 14, 12);
  if (LoadMod == 0) {
   vox.export(obj.BODYId + ".vox");
   vox = new VOX(voxSizex, voxSizey, voxSizez);
  }
  CreateVoxDetal(vox, obj.ARMS, obj.ARMSId, 11, 14, 12);
  if (LoadMod == 0) {
   vox.export(obj.ARMSId + ".vox");
   vox = new VOX(voxSizex, voxSizey, voxSizez);
  }
  CreateVoxDetalClone(vox, obj.ARMS, obj.ARMSId, 24, 14, 12, 4);
  if (LoadMod == 0) {
   vox.export(obj.ARMSId + "(1).vox");
   vox = new VOX(voxSizex, voxSizey, voxSizez);
  }
  CreateVoxDetal(vox, obj.HEAD, obj.HEADId, 15, 12, 24);
  if (LoadMod == 0) {
   vox.export(obj.HEADId + ".vox");
   zip.generateAsync({
    type: "blob"
 ...
 function CreateVoxDetal(vox, Detal, DetalName, posx, posy, posz) {
  for (var i = 0; i < Detal.length; i++) {
   var index = 0;
   var findcol = 0;
   for (var j = 0; j < vox.palette.length; j++) {
    if (vox.palette[j] == "0xff" + Detal[i].color) {
     index = j + 1;
     findcol++;
    }
   }
   if (findcol == 0) {
    vox.palette[val] = "0xff" + Detal[i].color;
    index = val + 1;
    val++;
   }
   vox.setVoxel(Detal[i].x + posx, Detal[i].y + posy, Detal[i].z + posz, index);
   var findcol = 0;
  }
 }
 function CreateVoxDetalClone(vox, Detal, DetalName, posx, posy, posz, sd) {
  for (var i = 0; i < Detal.length; i++) {
   var index = 0;
   var findcol = 0;
   for (var j = 0; j < vox.palette.length; j++) {
   ...
   vox.setVoxel(sd - Detal[i].x + posx, Detal[i].y + posy, Detal[i].z + posz, index);
   var findcol = 0;
  }
 }
}
...
var dynCall_viiifiiiiiiiii = Module["dynCall_viiifiiiiiiiii"] = createExportWrapper("dynCall_viiifiiiiiiiii");
